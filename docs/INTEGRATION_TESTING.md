...
Two exceptions now exist for extractor-regression coverage: [tests/fixtures/pdfs/fnb_regression_statement.pdf](tests/fixtures/pdfs/fnb_regression_statement.pdf) and [tests/fixtures/pdfs/absa_regression_statement.pdf](tests/fixtures/pdfs/absa_regression_statement.pdf) are committed fixtures paired with golden expected JSON files under [tests/fixtures/expected](tests/fixtures/expected). The API regressions in [tests/test_api_server.py](tests/test_api_server.py) upload those PDFs through the FastAPI endpoint, run the jobs inline, and compare the normalized line output to the expected JSON so missing FNB fee rows and missing ABSA repeated/page-break rows are caught immediately.
...
Exceptions: the committed FNB and ABSA regression fixtures above are intentionally versioned under `tests/fixtures/` because `teststatement/input/*.pdf` is reserved for manual runs and is gitignored.
